...
<template>
	<div id="basicWrapper" class="flex flex-col">
		<Backdrop class="bg-white/50 z-[1] backdrop-blur-sm" isClickable @click="hideBackdrop" />
		<TransitionDownToTop>
			<div v-if="isSearchShow" class="fixed w-screen top-0 left-0 z-[3]">
		...
		<main class="grow" :style="{ marginTop: navHeight + 'px' }">
			<slot name="main"></slot>
		</main>
...
<style scoped>
	...
	.nav-menu {
		@apply fixed left-0 -bottom-full w-full bg-white border border-neutral-300 border flex flex-col pb-8 pt-12 rounded-t-2xl z-[9999] lg:pb-0 lg:pt-0 lg:rounded-t-none lg:py-0 lg:h-auto lg:w-auto lg:static lg:bg-transparent lg:border-0 lg:flex-row lg:items-center lg:ml-auto transition-all duration-300 ease-in-out;
	}
